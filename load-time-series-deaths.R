...
csv = read.csv(file="../COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv", header=TRUE)

## row names are dates
deaths = data.frame(row.names=sub("X", "", colnames(csv)[-(1:4)]))

## interesting countries
deaths$Brazil = as.numeric(csv[csv$Country.Region=="Brazil",-(1:4)])
deaths$Denmark = as.numeric(csv[csv$Country.Region=="Denmark" & csv$Province.State=="Denmark", -(1:4)])
deaths$France = as.numeric(csv[csv$Country.Region=="France" & csv$Province.State=="France", -(1:4)])
deaths$Germany = as.numeric(csv[csv$Country.Region=="Germany",-(1:4)])
deaths$Iran  = as.numeric(csv[csv$Country.Region=="Iran", -(1:4)])
deaths$Italy = as.numeric(csv[csv$Country.Region=="Italy", -(1:4)])
deaths$Spain = as.numeric(csv[csv$Country.Region=="Spain", -(1:4)])
deaths$Sweden = as.numeric(csv[csv$Country.Region=="Sweden", -(1:4)])
deaths$SKorea = as.numeric(csv[csv$Country.Region=="Korea, South",-(1:4)])
deaths$UK    = as.numeric(csv[csv$Country.Region=="United Kingdom" & csv$Province.State=="United Kingdom", -(1:4)])
